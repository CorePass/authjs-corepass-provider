...
/**
 * CorePass store + pending + WebAuthn for MongoDB. Authenticators: collection with credential_id as unique key; index on user_id for list. See migrations/mongodb.
...
export function corepassMongoAdapter(opts: CorePassMongoAdapterOptions): CorePassStore & CorePassTx & {
	...
	const authenticators = client.collection(authenticatorsCollection)
	...
	return {
		...
		async getAuthenticator(credentialID: string): Promise<AdapterAuthenticator | null> {
			const row = await authenticators.findOne({ credential_id: credentialID })
			if (!row) return null
		...
		async listAuthenticatorsByUserId(userId: string): Promise<AdapterAuthenticator[]> {
			const rows = await authenticators.find({ user_id: userId }).limit(100)
