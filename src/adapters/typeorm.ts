...
/**
 * Minimal TypeORM DataSource-like client (raw query). Use with TypeORM DataSource.query().
 * @see https://authjs.dev/getting-started/database — TypeORM
 */
export type TypeORMLike = {
...
export type CorePassTypeORMAdapterOptions = {
	client: TypeORMLike
	schema?: string
}

/**
 * CorePass store + pending + WebAuthn for TypeORM (Postgres). Table: authenticators (see migrations/typeorm).
 * Merge with your Auth.js TypeORM adapter: adapter = { ...authAdapter, ...corepassTypeORMAdapter({ client: dataSource, schema }) }
 */
export function corepassTypeORMAdapter(opts: CorePassTypeORMAdapterOptions): CorePassStore & CorePassTx & {
	...
	return {
		...
		async updateAuthenticatorCounter(credentialID: string, newCounter: number): Promise<AdapterAuthenticator> {
			...
			await execute(`UPDATE ${pre}authenticators SET counter = $1 WHERE credential_id = $2`, [
				newCounter,
				credentialID,
			])
